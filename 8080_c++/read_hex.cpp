...
//For an error call for an invalid instruction/opcode
...
void Emulate8080Op(State8080* state){
	unsigned char *opcode = &state->memory[state->pc]; //It's a pointer that points to the PC

	switch(*opcode){
		case 0x00:
			break;
		case 0x01:
			state->pc = opcode[1]; //LXI   B, word == Load the register pair immediate. So the address of the second part, i.e, word is copied to the B(L?) pair
			state->pc = opcode[2];
			state->pc += 2;
			break;
	}
	state->pc += 1; //for the opcode? so add 1 to the PC to get the next opcode?
}
